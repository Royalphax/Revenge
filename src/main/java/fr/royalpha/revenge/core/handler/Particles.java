...
public enum Particles {
	...
	SPELL_MOB("mobSpell", 15, 3), 
	SPELL_MOB_AMBIENT("mobSpellAmbient", 16, 3), 
	...
	public static class RevengeParticle {
		...
		public RevengeParticle(RevengePlugin instance, String input) {
			...
			try {
				...
				ex.printStackTrace();
				instance.getLogger().info("[An error occurred, please contact the developer] Particle error: " + ex.getMessage());
			}
			/*instance.getLogger()
		...
		public void playParticles(Location location) {
			...
			if (asRGBParticle) {
				for (int i = 0; i < particleCount; i++)
					RevengePlugin.getVersionManager().getParticleFactory().playParticles(hitParticle, location, (float) color(particleArg1), (float) color(particleArg2), (float) color(particleArg3), 
							0, (float) 1.0, new int[0]);
			} else if (asItemCrackParticle) {
				RevengePlugin.getVersionManager().getParticleFactory().playParticles(hitParticle, location, 0F, 0F, 0F,
		...
		public void playBloodParticles(Location location) {
			RevengePlugin.getVersionManager().getParticleFactory().playParticles(Particles.BLOCK_CRACK, location, 0.0f, 0.0f, 0.0f, 1, 0.1f, 152);
		}
