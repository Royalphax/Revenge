	...
	public void playParticles(Particles particle, Location location, Float fx, Float fy, Float fz, int amount,
		...
		if (particle.getOptionalParametersSize() > 0) {
			if (!triedParticles.contains(particle)) {
				...
				Bukkit.getLogger().info("<!> Something tried to show " + particle.toString() + " particle to a player. Sorry but this particle can't be used yet ! As you may know, 1.14 is really recent and our developers are always looking for how to handle this type of particle. Will be fixed in another update."); 
			}
		} else {
